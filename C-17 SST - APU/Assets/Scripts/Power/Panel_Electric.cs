...
{
    // Debugging
    public bool Debug_External_Avail = true;
    public bool Debug_APU_Avail = true;
    ...
    public int sw_APU = 1;                     // 0 - Reset, 1 - Off, 2 - On
    float sw_APU_timer = 0.0f;
    int sw_APUreset = 0;
    ...
    {
        if (apu_manager.status_StartSeq_Step == 5) APU_Avail = true;
        else APU_Avail = false;
        ...
        {
            if(APU_On) Ind_APU.SetMaterial(2);
            else Ind_APU.SetMaterial(1);
        }
        else
        {
        ...
        {
            if (APU_Avail || Debug_APU_Avail)
            {
                if(sw_APU == 1) Ind_APU.SetMaterial(1);
                else if (sw_APU == 2) Ind_APU.SetMaterial(2);
            }
            else Ind_APU.SetMaterial(0);
    ...
    public void ToggleAPUPower_Down()
    {
        if (sw_APU == 0)
        {
            sw_APU++;
